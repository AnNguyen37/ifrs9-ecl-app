...
ecl_s1_m = fmt_money(summary['total_ecl_stage1'], 'M')
ecl_s2_m = fmt_money(summary['total_ecl_stage2'], 'M')
exposure_b = fmt_money(summary['total_exposure'], 'B')

st.success(
    f"**A fully functional IFRS 9 ECL engine** built on Lending Club's unsecured "
    f"retail lending portfolio ({int(summary['n_loans']):,} loans, {exposure_b} exposure).\n\n"
    f"- **Stage 1 provision (12-month):** {ecl_s1_m} ({summary['coverage_stage1']*100:.2f}% coverage)\n"
    f"- **Stage 2 provision (lifetime):** {ecl_s2_m} ({summary['coverage_stage2']*100:.2f}% coverage)\n\n"
    f"Each component (PD, LGD, EAD, timing) was tested, justified, and documented "
    f"following industry practice (GARP) and regulatory guidance (IFRS 9, CRR3)."
)
...
with st.expander("👤 About this project"):
    ...
    **Code and documentation:** https://github.com/AnNguyen37/ifrs9-ecl-app

    **Live app:** https://ifrs9-ecl-an.streamlit.app/
