...
default risk captured under lifetime ECL. The Stage 2 provision would activate for loans 
that experience a Significant Increase in Credit Risk (SICR) — typically 30+ days past due.
""")
...
When a 60-month loan triggers SICR (moves to Stage 2), the provision increase is 
proportionally larger because more remaining months are now in scope. This is why 
SICR triggers on long-term loans have a disproportionate provisioning impact — 
a critical consideration for forward-looking IFRS 9 risk management.
""")
...
if horizon == 12:
    ...
    st.warning(f"**Stage 2 territory** — {horizon}-month horizon, beyond the SICR threshold")
...
with st.expander("📘 ECL Methodology Summary"):
    ...
    |---|---|---|
    | Stage 1 | No SICR | 12 months |
    | Stage 2 | SICR (30+ DPD) | Lifetime |
    | Stage 3 | Default (90+ DPD) | Lifetime, PD = 100% |
